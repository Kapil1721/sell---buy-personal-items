              ...
              <div className=" rounded-b-lg  transition group-hover:shadow-lg overflow-hidden border-b border-[#F2F4F8] bg-[#FDFDFE]">
                <div className="grid grid-cols-2 justify-between items-center px-3  py-4">
                  <div className="grid grid-cols-3 items-center gap-4">
                    <Link to={`/products/${product.slug}`}><TooltipIcon IconComponent={ViewIcon} tooltipText="View" id={`View-${product.post_id}`} /></Link>
                    {/* <TooltipIcon IconComponent={CompareIcon} tooltipText="Compare" id={'Compare'} /> */}
                    {user && <TooltipIcon IconComponent={LikeIcon} tooltipText="Like" id={`Like-${product.post_id}`} like={product.likeStatus} onClick={() => handleLike(product.post_id)} />}
                  </div>
