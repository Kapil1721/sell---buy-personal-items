...
export default function RatingComponent({ rating, onChange }) {
    const [stars, setStarts] = useState(rating ? rating : 0);
    const currentStars = typeof rating === "number" ? rating : stars;
                ...
                {Array.from({ length: 5 }).map((_, index) => (
                    <div key={index} className="flex justify-center">
                        ...
                        <svg
                            ...
                            //     }
                            // }
                            fill={index + 1 <= currentStars ? "gold" : "#d5e3ee"}
                            className={index + 1 <= currentStars ? "transition ease-in-out duration-500 hover:fill-secondary" : " transition ease-in-out duration-500"}
                        >
            ...
            </div>
            {
                currentStars > 0 ? (
                    <div
                        ...
                        style={{ color: ratingData[currentStars - 1]?.color }}>
                        {ratingData[currentStars - 1]?.label}
                    </div>
                ) : (
